...
2. `.attention-diff/runs/<runId>/` に `diff.json` を作成します。
...
通常のrunでは `diff.json` と `attention.json` だけを残します。parserやpromptを調査したい場合は、次のように依頼または実行すると `raw.diff` などの中間成果物も残せます。

```text
/attention-diff 190 --keep-artifacts
```
